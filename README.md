...
> ## ⚠️ 【必須】スクリプトを走らせる前に必ずやること：PAYGへのアップグレード
>
> **Free Tierのままでは、スクリプトが空きを見つけても取得できません。**
> ARMインスタンスはFree Tierアカウントには割り当て優先度が低く、実質ほぼ取れません。
>
> ### 手順（5分で完了）
> 1. [OCIコンソール](https://cloud.oracle.com/) にログイン
> 2. 左上ハンバーガーメニュー（≡）→ **「Billing & Cost Management」**
> 3. 左メニュー → **「Upgrade and Manage Payment」**
> 4. **「Change Payment Method」** → **「Credit Card」** → カード情報を入力
>    - 登録時に**100円前後の仮引き落とし**が発生しますが、すぐ返金されます
...
> ## 📍 リージョンについて：東京（ap-tokyo-1）でOK
>
> 「東京は取れない」という情報を目にすることがありますが、それは**Free Tierの話**です。
> **PAYGにアップグレード済みであれば、東京リージョンを狙い続けて問題ありません。**
>
> - `OCI_REGION` は `ap-tokyo-1` のまま変更不要
> - リージョンを変えるにはアカウント作り直しが必要なため、今の設定を維持すること
> - 北米リージョンに逃げる必要もなし（AItuber・Botなどの用途ならレイテンシも関係ない）
...
>    - **Threshold Amount**: `1`
> 7. **Email Recipients** に通知を受け取りたいメールアドレスを入力
> 8. 右下の **「Create」** をクリック
>
> ### これで何が変わる？
